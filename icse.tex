...
They usually rely on planners that given a robot or a team of robots computes plans that specify how the robot(s) can achieve their missions.
...
This paper presents \toolName\ a tool that aims supporting run-time mission execution by tackling the previous challenges.
\toolName\ provides a decentralized planning solution and  is able to work also when only partial knowledge of the environment is present.
...
Partial knowledge is handled by executing a classical planning algorithm twice.
\end{abstract}
