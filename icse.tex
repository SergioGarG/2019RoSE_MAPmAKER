...
\claudio{Read your sentence and compare it with the following:
Environments where robots are deployed are  dynamic and change over time, e.g.,
doors can be open and closed, humans can move between different rooms.
For this reason environments are usually \emph{partially known}.
}


Our proposed tool, \toolName\ tackle the limitations that current planning techniques are used for teams of robots: 
\claudio{Why which limitations? you never mentioned that current tools have limitations.
Maybe you should add a sentence at the end of the previous paragraph such as:
However, current solutions  .. and something about the fact that do not address partially known environments and are not decentralized.}
\begin{enumerate*}
...
\section{Limitations of current planners}
