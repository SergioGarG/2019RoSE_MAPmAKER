...
\begin{abstract}
Robot applications are being increasingly used in order to help humans taking dangerous, heavy or monotonous tasks that can be handled without expertise.
A robot application can be composed by a robot or a team of them if the user wants them to perform a more complex mission.
For these reasons, those applications ask for decentralized techniques that allow for \emph{tractable} automated planning. 
\claudio{A way to increment the efficiency of planning is by using decentralized techniques which generally decompose a complex mission over the team of robots.}
Furthermore, environments where robots are deployed are dynamic and change over time, e.g.,
doors can be open and closed, humans can move between different rooms.
Thus, environments are usually \emph{partially known}.
However, current solutions with decentralized computation do not address such environments.

Our proposed tool, \toolName\ tackle the limitations that current planning techniques are used for teams of robots: 
\begin{enumerate*}
\item it decomposes the robotic team into subclasses \claudio{based on the mission to be achieved}, avoiding the not scalable centralized approach;\claudio{I would switch points 1 and 2.}
...
\claudio{Never mentioned what a possible plan is, i would remove ``performing possible plans"}.
\end{enumerate*}
