...
The first option is used for computing plans on environment and robots custom models and missions.
...
With the second option we provide a way of replicating the experiments showed in this paper and in~\cite{mapmaker17}.
...
In both paths the robot reaches the cell where the requested action can be performed with the help of robot $r_2$.
For robot $r_2$ we show two different paths as well.
P2 represents a possible plan ---due to the uncertainty of the transition between cells 14 and 20--- where $r_2$ must synchronize with $r_1$ in order to accomplish action 2.
P2' is a similar path with the difference that the synchronization is not assured in the target cell.
